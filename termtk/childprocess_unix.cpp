...
struct ChildProcessImpl {
  ...
  void Kill() {
    if (::kill(child_pid_, SIGKILL) != 0) {
      std::cout << "fail to kill: " << child_pid_ << " => " << errno
                << std::endl;
    }
  }

  void Write(const char *buf, size_t size) { write(pty_fd_, buf, size); }
...
ChildProcess::~ChildProcess() { delete impl_; }

void ChildProcess::Launch(int rows, int cols, const char *prog,
                          const std::vector<std::string> &args,
                          const char *TERM) {
  impl_->Launch(rows, cols, prog, args, TERM);
}
